...
link = ["https://environment.data.gov.uk/flood-monitoring/id/stations/46106/readings?since=",dayref, "&_sorted&parameter=rainfall"]
final = ""
...
#reg = reg.predict([[1.3]])
plt.plot(tester['Rain'], reg.predict(tester[['Rain']]), color = 'r')
plt.show()

#first testing boys
initial = b['River Level'].iloc[0]
predictions = pd.DataFrame(reg.predict(b[['Rainfall']]))
